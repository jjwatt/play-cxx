...
template <int width_array_length, int height_array_length>
void DrawPlasma(SDL_Renderer *r, const std::array<std::array<double, width_array_length>, height_array_length> &array,
		const double &hue_shift) {
    for (unsigned long y = 0; y < array.size(); ++y)
	for (unsigned long x = 0; x < array.at(0).size(); ++x) {
	    // Convert the HSB value to the RGB value
	    double Hue = hue_shift + std::fmod(array[y][x], 1);
	    RGB CurrentColor = HSBToRGB(Hue, 1, 1);
	    // Draw the actual plasma
	    SDL_SetRenderDrawColor(r, CurrentColor.Red, CurrentColor.Green, CurrentColor.Blue, 0xff);
	    SDL_RenderDrawPoint(r, x, y);
	}
}

int main(void) {
    constexpr unsigned DefaultWidth  = 640,
	               DefaultHeight = 640;
    ...
    Window = SDL_CreateWindow("Plasma Effect", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
    ...
    // The window runs until the "quite" flag is set to true
    while(!KillWindow) {
